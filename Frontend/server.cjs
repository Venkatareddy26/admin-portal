...
// Optional Postgres persistence: if DATABASE_URL or PGHOST is set, we'll
// keep the existing file-backed storage but also mirror writes to Postgres
// and on startup load DB contents into the JSON files so the app reflects
// the backend in realtime.
let pool = null;
try {
  const pg = require('pg');
  const hasPgCfg = !!(process.env.DATABASE_URL || process.env.PGHOST || process.env.PGHOSTADDR);
  if (pg && hasPgCfg) {
    const Pool = pg.Pool || pg.Client || pg.default?.Pool;
    pool = new Pool(process.env.DATABASE_URL ? { connectionString: process.env.DATABASE_URL } : undefined);
  }
} catch (e) {
  // pg not installed or not configured; we'll operate in file-only mode.
  pool = null;
}
...
async function ensureStoreTable() {
  ...
  } catch (e) { console.error('failed to ensure app_store table', e); }
}

async function dbUpsert(ns, value) {
  if (!pool) return;
  try {
    await pool.query(
      'INSERT INTO app_store (ns, data) VALUES ($1,$2) ON CONFLICT (ns) DO UPDATE SET data = EXCLUDED.data, updated_at = now()',
      [ns, value]
    );
  } catch (e) { console.error('dbUpsert error for', ns, e); }
}
...
async function syncDbToFiles() {
  ...
  try {
    let res;
    try {
      res = await pool.query('SELECT ns, data FROM app_store');
    } catch (err) {
      // relation does not exist -> create and retry once
      if (err && err.code === '42P01') {
        console.warn('app_store table missing, attempting to create it');
    ...
    for (const row of res.rows) {
      ...
      try { fs.writeFileSync(p, JSON.stringify(row.data || [], null, 2)); }
      catch (e) { console.error('failed to write file from db', p, e); }
    }
  } catch (e) { console.error('syncDbToFiles failed', e); }
}
...
function writeJSON(name, v){ const p = path.join(DATA_DIR, name); fs.writeFileSync(p, JSON.stringify(v, null, 2));
  ...
  try { dbUpsert(name, v); } catch(e) { /* ignore */ }
}
